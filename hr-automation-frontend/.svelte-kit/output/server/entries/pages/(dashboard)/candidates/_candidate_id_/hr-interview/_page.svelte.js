...
import { c as useStartHRInterview } from "../../../../../../chunks/interviews.js";
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const fullInterviewUrl = (() => {
      ...
      return /^https?:\/\//i.test(sessionInfo.interview_url) ? sessionInfo.interview_url : `http://localhost:8200${sessionInfo.interview_url}`;
    })();
    const eligible = candidate?.technical_test_result === "pass" && !(candidate?.hr_test === true && candidate?.hr_test_result);
    ...
    async function generateLink() {
      ...
      if (candidate.hr_test && candidate.hr_test_result) {
        toast.error(`Candidate already completed HR interview (result: ${candidate.hr_test_result}). Use "Reset & Regenerate" first.`);
        return;
      }
      generating = true;
      try {
    ...
    async function resetAndRegenerate() {
      if (!candidate) return;
      const confirmed = window.confirm("This will delete the previous HR interview and create a fresh session. Continue?");
      if (!confirmed) return;
      regenerating = true;
      try {
        const r = await fetchWithAuth(`${API_URLS.main}/hr-interview/reset/${candidate.candidate_id}`, { method: "POST" });
    ...
    head("1be5n2s", $$renderer2, ($$renderer3) => {
      $$renderer3.title(($$renderer4) => {
        $$renderer4.push(`<title>Schedule HR Interview | HR Automation</title>`);
      });
    });
    ...
    Handshake($$renderer2, { class: "w-6 h-6 text-purple-500" });
    $$renderer2.push(`<!----> <h1 class="text-2xl font-bold text-gray-900">Schedule HR Interview</h1></div> <p class="text-gray-500">Generate a unique HR interview link for <span class="font-medium text-gray-700">${escape_html(candidate ? `${candidate.first_name ?? ""} ${candidate.last_name ?? ""}`.trim() : "…")}</span> and share it with the candidate.</p></div></div> `);
      ...
      if (!candidate) {
        ...
        if (candidate.technical_test_result !== "pass") {
          ...
          Card($$renderer2, {
            ...
            children: ($$renderer3) => {
              $$renderer3.push(`<div class="w-14 h-14 mx-auto rounded-full bg-purple-100 flex items-center justify-center mb-3">`);
              Handshake($$renderer3, { class: "w-7 h-7 text-purple-600" });
              $$renderer3.push(`<!----></div> <h3 class="text-lg font-medium text-gray-800 mb-2">Not eligible yet</h3> <p class="text-gray-500 mb-4">Candidate must pass the technical interview before an HR interview can be scheduled.</p> <p class="text-sm text-gray-400">Current technical result: <span class="font-medium">${escape_html(candidate.technical_test_result || "not taken")}</span></p>`);
            }
          });
        } else {
          $$renderer2.push("<!--[!-->");
          if (!sessionInfo) {
            ...
            Card($$renderer2, {
              ...
              children: ($$renderer3) => {
                Handshake($$renderer3, { class: "w-16 h-16 mx-auto text-gray-400 mb-4" });
                $$renderer3.push(`<!----> <h3 class="text-lg font-medium text-gray-700 mb-2">No interview link yet</h3> <p class="text-gray-500 mb-6 max-w-md mx-auto">Generate a one-click HR interview link. The candidate logs in with their email and password,
        and the AI HR interviewer starts automatically.</p> `);
                Button($$renderer3, {
            ...
            Card($$renderer2, {
              class: "p-6",
              children: ($$renderer3) => {
                $$renderer3.push(`<div class="flex items-start gap-4"><div class="w-14 h-14 rounded-xl bg-purple-100 flex items-center justify-center flex-shrink-0">`);
                Handshake($$renderer3, { class: "w-7 h-7 text-purple-600" });
                $$renderer3.push(`<!----></div> <div class="flex-1"><div class="flex items-center gap-3 mb-2"><h3 class="text-xl font-semibold text-gray-900">HR Interview Ready</h3> <span class="px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs font-medium">Active</span></div> <p class="text-sm text-gray-500">Active session for <span class="font-medium text-gray-700">${escape_html(candidate ? `${candidate.first_name ?? ""} ${candidate.last_name ?? ""}`.trim() : "candidate")}</span></p></div></div>`);
              }
            });
            $$renderer2.push(`<!----> `);
            Card($$renderer2, {
            ...
            Card($$renderer2, {
              class: "p-6",
              children: ($$renderer3) => {
                $$renderer3.push(`<h4 class="font-medium text-gray-900 mb-4">How It Works</h4> <ol class="space-y-3 text-sm text-gray-500"><li class="flex items-start gap-3"><span class="w-6 h-6 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center flex-shrink-0 text-xs font-medium">1</span> <span>Copy the <strong class="text-gray-700">Candidate Interview URL</strong> and share it with the candidate.</span></li> <li class="flex items-start gap-3"><span class="w-6 h-6 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center flex-shrink-0 text-xs font-medium">2</span> <span>Candidate opens the URL and logs in using their <strong class="text-gray-700">email + password</strong> shown above.</span></li> <li class="flex items-start gap-3"><span class="w-6 h-6 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center flex-shrink-0 text-xs font-medium">3</span> <span>The AI HR interviewer conducts a behavioural conversation.</span></li> <li class="flex items-start gap-3"><span class="w-6 h-6 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center flex-shrink-0 text-xs font-medium">4</span> <span>Score and detailed AI feedback appear on the candidate's profile automatically.</span></li></ol>`);
              }
            });
