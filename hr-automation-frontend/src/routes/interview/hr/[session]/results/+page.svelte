<script lang="ts">
  ...
  import { useHRInterviewBySession } from '$lib/api/queries/interviews';
  ...
  // Score breakdown
  const culturalFitScore = $derived(interview?.cultural_fit_score || 0);
  const communicationScore = $derived(interview?.communication_score || 0);
  const motivationScore = $derived(interview?.motivation_score || 0);
  const professionalismScore = $derived(interview?.professionalism_score || 0);
  const teamworkScore = $derived(interview?.teamwork_score || 0);
  const leadershipScore = $derived(interview?.leadership_score || 0);
  ...
  const jobTitle = $derived(interview?.job_details?.title || 'HR Interview');
  ...
  <header class="bg-white/80 backdrop-blur-sm border-b border-gray-200/60 sticky top-0 z-10">
    <div class="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="font-semibold text-gray-800">HR Interview</h1>
  ...
  <main class="max-w-5xl mx-auto px-4 py-8">
    ...
    {:else if interview}
      <div class="space-y-6">
            ...
            <div class="flex items-center gap-6">
              ...
              <div>
                ...
                <p class="text-gray-500">
                  HR assessment completed and evaluated by AI
                </p>
              </div>
            </div>
        ...
        <Card class="p-6">
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="text-center p-4 rounded-xl bg-gray-50">
              <ScoreGauge score={culturalFitScore} />
              <p class="mt-3 text-sm font-medium text-gray-700">Cultural Fit</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-gray-50">
            ...
            <div class="text-center p-4 rounded-xl bg-gray-50">
              <ScoreGauge score={motivationScore} />
              <p class="mt-3 text-sm font-medium text-gray-700">Motivation</p>
            </div>
            <div class="text-center p-4 rounded-xl bg-gray-50">
              <ScoreGauge score={professionalismScore} />
              <p class="mt-3 text-sm font-medium text-gray-700">Professionalism</p>
            </div>
          </div>
          ...
          {#if teamworkScore > 0 || leadershipScore > 0}
            <div class="grid sm:grid-cols-2 gap-6 mt-4">
              {#if teamworkScore > 0}
                <div class="text-center p-4 rounded-xl bg-gray-50">
                  <ScoreGauge score={teamworkScore} />
                  <p class="mt-3 text-sm font-medium text-gray-700">Teamwork</p>
                </div>
              {/if}
              {#if leadershipScore > 0}
                <div class="text-center p-4 rounded-xl bg-gray-50">
                  <ScoreGauge score={leadershipScore} />
                  <p class="mt-3 text-sm font-medium text-gray-700">Leadership</p>
                </div>
              {/if}
            </div>
          {/if}
        ...
        {#if passed}
          <Card class="p-6 bg-gradient-to-r from-emerald-50 to-green-50 border-emerald-200">
            <div class="flex items-center gap-4">
              <div class="w-12 h-12 rounded-xl bg-emerald-100 flex items-center justify-center">
                <Sparkles class="w-6 h-6 text-emerald-600" />
              </div>
              <div>
                <h3 class="font-semibold text-emerald-800">Ready for Hiring Decision</h3>
                <p class="text-sm text-emerald-600">This candidate has successfully completed all interview rounds and is recommended for hire.</p>
              </div>
            </div>
          </Card>
        {/if}
    ...
    {:else}
      <Card class="p-8 text-center">
        <div class="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-4">
          <Handshake class="w-8 h-8 text-gray-400" />
        </div>
